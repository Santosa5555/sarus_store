...
const HeroListProducts = () => {
    ...
    const { data, loading, error } = useFetch(`https://dummyjson.com/products?limit=5&skip=${count === 0 ? 0 : count*10} `)
    ...
    return (
        ...
        <div className="">
            ...
            <div className="button text-center">
                <button disabled={disableButton} onClick={() =>setCount(prevCount => prevCount + 1)} className={`text-xl text-rose-600 border-2 w-80 m-10 border-gray-800 text0 ${disableButton?"hover:text-xl ":"hover:text-lg"}`}>LOAD MORE</button>
            </div>
        </div>
    )
}
